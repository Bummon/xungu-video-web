...
} from "@/stores/modules/chartHistoryStore/chartHistoryStore.d";
...
export const useChartEditStore = defineStore({
  ...
  getters: {
    ...
    //   获取布局的标题名字
    getTitle(): string {
      return this.editCanvasConfig.projectName as string;
    }
  },
  actions: {
    // 硬刷新逻辑  先不使用
    setAll(componentList: any, requestGlobalConfig: any, editCanvasConfig: any) {
      for (let i in editCanvasConfig) {
        this.setEditCanvasConfig(i as keyof EditCanvasConfigType, editCanvasConfig[i]);
      }
      console.log("赋值断点1");
      console.log("editCanvasConfig", this.editCanvasConfig);
      this.setEditCanvasConfig(EditCanvasConfigEnum.PROJECT_NAME, editCanvasConfig.projectName);
      console.log("赋值断点1", this.editCanvasConfig.projectName);
      this.addComponentList(componentList, false, false); //插到图层列表里
     ...
     */
    addComponentList(
      componentInstance: CreateComponentType | CreateComponentGroupType | Array<CreateComponentType | CreateComponentGroupType>,
      isHead = false,
      isHistory = false
    ): void {
    ...
    setBothEnds(isEnd = false, isHistory = true): void {
      try {
        ...
        console.error("移动组件列表层级位置到两端出错！", value);
      }
    },
    // * 置顶
    ...
    wrap(isDown = false, isHistory = true) {
      try {
        ...
        if (index !== -1) {
          ...
          // 历史记录
          if (isHistory) {
            chartHistoryStore.createLayerHistory([targetItem], isDown ? HistoryActionTypeEnum.DOWN : HistoryActionTypeEnum.UP);
          }
        ...
        loadingError("上移/下移互换图表位置出错", value);
      }
    },
    // * 图层上移
    ...
    setLock(status: boolean = true, isHistory: boolean = true) {
      try {
        ...
        if (index !== -1) {
          ...
          // 历史记录
          if (isHistory) {
            status ? chartHistoryStore.createLockHistory([targetItem]) : chartHistoryStore.createUnLockHistory([targetItem]);
          }
    ...
    setHide(status: boolean = true, isHistory: boolean = true) {
      try {
        ...
        if (index !== -1) {
          ...
          // 历史记录
          if (isHistory) {
            status ? chartHistoryStore.createHideHistory([targetItem]) : chartHistoryStore.createShowHistory([targetItem]);
          }
          this.updateComponentList(index, targetItem);
          loadingFinish();
