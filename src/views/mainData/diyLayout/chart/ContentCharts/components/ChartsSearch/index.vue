...
import { ChartModeEnum, ChartLayoutStoreEnum } from "@/stores/modules/chartLayoutStore/chartLayoutStore.d";
...
const searchHandle = (key: string | null) => {
  ...
  searchRes.value = List.filter((e: ConfigType) => !e.disabled && (!key || e.title.toLowerCase().includes(key.toLowerCase())));
  setTimeout(() => {
    loading.value = undefined;
  }, 500);
};
