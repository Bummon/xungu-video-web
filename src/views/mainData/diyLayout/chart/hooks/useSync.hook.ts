...
import { ChartEditStoreEnum, ChartEditStorage } from "@/stores/modules/chartEditStore/chartEditStore.d";
  ...
  const updateComponent = async (projectData: ChartEditStorage, isReplace = false, changeId = false) => {
    ...
    const create = async (
      ...
      if (callBack) {
        ...
        if (changeId) {
          chartEditStore.addComponentList(componentMerge(newComponent, { ..._componentInstance, id: getUUID() }), false, true);
        } else {
