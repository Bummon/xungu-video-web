...
const handleContextMenu = (
  ...
  console.log("右键目标", targetInstance?.id);
  ...
  nextTick().then(() => {
    console.log("清洗完数据后，dom刷新完毕，然后右键弹窗,窗呢？");
...
export const useContextMenu = () => {
  ...
  const handleMenuSelect = (key: string) => {
    ...
    menuOptions.value.forEach((e: MenuOptionsItemType) => {
      if (e.key === key) {
        ...
        if (!targetItem) loadingError("// * 事件处理出错", "handleMenuSelect函数");
      }
    });
  };
  ...
  return {
    ...
    handleContextMenu, // 右键处理
    onClickOutSide, //  * 失焦
    handleMenuSelect, // 处理事件
