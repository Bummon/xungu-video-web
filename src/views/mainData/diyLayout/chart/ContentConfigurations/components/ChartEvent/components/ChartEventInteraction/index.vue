<template>
  <n-collapse-item title="组件交互" name="1" v-if="interactActions.length">
    ...
    <n-card v-for="(item, cardIndex) in targetData.events.interactEvents" :key="cardIndex" class="n-card-shallow" size="small">
      ...
      <setting-item-box :name="requestParamsItem" v-for="requestParamsItem in requestParamsTypeList" :key="requestParamsItem">
        <setting-item
...
const fnEventsOptions = (): Array<SelectOption | SelectGroupOption> => {
  ...
  const fnFlattern = (
    ...
    return data.reduce(
      (iter: Array<CreateComponentType | CreateComponentGroupType>, val: CreateComponentType | CreateComponentGroupType) => {
