...
const columns: ColumnProps<BaseRoom.Room>[] = [
  ...
  { prop: "billTitle", label: "清单名称", align: "left", width: TableWidthEnum.Name, search: { el: "input" } },
  { prop: "billDescr", label: TableLabelEnum.Descr, align: "left" },
  {
    prop: "billStatus",
    label: "审核状态",
    align: "left",
  ...
  { prop: "createUserName", label: "创建者", width: TableWidthEnum.PersonName, align: "left", search: { el: "input" } },
  { prop: "operation", label: TableLabelEnum.Operation, width: TableWidthEnum.Handle3, fixed: "right" }
];
// render: scope => {
//   let arr = ["草稿", "待审核", "已审核", "已发布", "已完成", "作废"];
//   return arr[scope.row.billStatus];
// }
// 如果表格需要初始化请求参数，直接定义传给 ProTable(之后每次请求都会自动带上该参数，此参数更改之后也会一直带上，改变此参数会自动刷新表格数据)
const initParam = reactive({ billType: 1, pageSize: 10 });
...
const submitAudit = (row: Partial<BaseRoom.Room> = {}) => {
